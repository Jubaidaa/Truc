...
static void build_filepath(char *filepath, size_t size,
  ...
  if (strcmp(request->target->data, "/") == 0) {
    snprintf(filepath, size, "%s/%s", config->root_dir, config->default_file);
  } else {
    snprintf(filepath, size, "%s%s", config->root_dir, request->target->data);
  }
}
...
void server_run_http(struct server *server) {
  ...
  while (server->running && g_server_running) {
    ...
    inet_ntop(AF_INET, &client_addr.sin.sin_addr, client_ip, INET_ADDRSTRLEN);
...
void server_run(struct server *server) { server_run_http(server); }
