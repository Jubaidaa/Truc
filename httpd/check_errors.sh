...
test_fail() {
    ...
    # Exécute le binaire avec les arguments donnés, redirige stderr vers /dev/null
    $BINARY "$@" 2>/dev/null
    ret=$?
...
# 1. Manque le Port (on met tout le reste)
...
# 6. Option inconnue (doit aussi renvoyer 2 selon getopt)
